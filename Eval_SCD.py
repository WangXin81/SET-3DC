...
def Eval():
    ...
    assert set([os.path.basename(label).split(".")[0] for label in label_list1]) == set(
        [os.path.basename(infer).split(".")[0] for infer in infer_list1]), "Predictions do not match targets name"
    assert set([os.path.basename(label).split(".")[0] for label in label_list2]) == set(
        [os.path.basename(infer).split(".")[0] for infer in infer_list2]), "Predictions do not match targets name"
    ...
    print('change_ratio = %.4f, SC_Precision = %.4f, SC_Recall = %.4f, F_scd = %.4f' % (
    change_ratio * 100, SC_Precision * 100, SC_Recall * 100, F1 * 100))
