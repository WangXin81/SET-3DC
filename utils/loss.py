...
def weighted_BCE_logits(logit_pixel, truth_pixel, weight_pos=0.28, weight_neg=0.72):
...
class SCA_Loss(nn.Module):
    ...
    def forward(self, p1, p2, gt_mask):   
        ...
        losses = self.loss_f(p1_change.contiguous().view(b,-1),p2_change.contiguous().view(b,-1))-self.loss_f(p1_unchange.contiguous().view(b,-1),p2_unchange.contiguous().view(b,-1))+1
