...
# from datasets import RS_ST_2 as RS
# from models.BiSRNet import BiSRNet as Net
# from models.cdsc import cdsc as Net
# from models.HRSCD3 import HRSCD3 as Net
from models.SET_3DC  import swin_T_3D as Net
# 生成一个随机5位整数
import math
...
def train(train_loader, net, criterion, optimizer, scheduler, val_loader):
    ...
    curr_epoch = 0
    # checkpoint_path = os.path.join(args['chkpt_dir'], 'SET_3DC_29e_mIoU67.07_Sek9.88_Fscd58.66_OA90.85_Pre58.47_Rec58.84_sroce27.04.pth')
    ...
    while True:
        ...
        for i, data in enumerate(train_loader):
            ...
            loss_seg = criterion(outputs_A * labels_bn, labels_A) * 0.5 + criterion(outputs_B * labels_bn,
                                                                                    labels_B) * 0.5
            ...
            if (i + 1) % args['print_freq'] == 0:
                print(
                    '[epoch %d] [iter %d / %d %.1fs] [lr %f] [train seg_loss %.4f bn_loss %.4f sc_loss %.4f acc %.2f]' % (
                        curr_epoch, i + 1, len(train_loader), curr_time, optimizer.param_groups[0]['lr'],
                        train_seg_loss.val, train_bn_loss.val, train_sc_loss.val,
                        acc_meter.val * 100))  # sc_loss %.4f, train_sc_loss.val,
        ...
        Fscd_v, mIoU_v, Sek_v, acc_v, loss_v, Pre, Rec, score, Sek0, Sek1, mSek = validate(val_loader, net, criterion,
                                                                                           curr_epoch)
        ...
        if score > bestSroce:
            ...
            torch.save(net.state_dict(), os.path.join(args['chkpt_dir'],
                                                      NET_NAME + '_%de_mIoU%.2f_Sek%.2f_Fscd%.2f_OA%.2f_Pre%.2f_Rec%.2f_sroce%.2f.pth' \
                                                      % (
                                                      curr_epoch, mIoU_v * 100, Sek_v * 100, Fscd_v * 100, acc_v * 100,
                                                      Pre * 100, Rec * 100, score * 100)))
        print('Total time: %.1fs Best rec: Train acc %.2f, Val Fscd %.2f acc %.2f loss %.4f' % (
        time.time() - begin_time, bestaccT * 100, Fscd_v * 100, bestaccV * 100, bestloss))
        curr_epoch += 1
        # scheduler.step()
...
def validate(val_loader, net, criterion, curr_epoch):
    ...
    Fscd, IoU_mean, Sek, sorce, PRE, REC, Sek0, Sek1, mSek = SCDD_eval_all(preds_all, labels_all, RS.num_classes)

    curr_time = time.time() - start
    print(
        '%.1fs Val loss: %.2f Fscd: %.2f IoU: %.2f Sek: %.2f Accuracy: %.2f Pre: %.2f Rec:%.2f, sroce:%.2f, Sek0:%.2f, Sek1:%.2f, mSek:%.2f' \
        % (curr_time, val_loss.average(), Fscd * 100, IoU_mean * 100, Sek * 100, acc_meter.average() * 100, PRE * 100,
           REC * 100, sorce * 100, Sek0 * 100, Sek1 * 100, mSek * 100))
    ...
    return Fscd, IoU_mean, Sek, acc_meter.avg, val_loss.avg, PRE, REC, sorce, Sek0, Sek1, mSek
